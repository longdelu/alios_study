...
const u8 *tcp_client_sendbuf="TCP Client send data\r\n";
...
void tcp_demo_init(void)
{
    ...
    if(tcppcb) {         //创建成功
        ...
        IP4_ADDR(&rmtipaddr,lwipdev.remoteip[0],lwipdev.remoteip[1],lwipdev.remoteip[2],lwipdev.remoteip[3]); 
         res = tcp_connect(tcppcb,&rmtipaddr,TCP_CLIENT_PORT,tcp_client_connected);  //连接到目的地址的指定端口上,当连接成功后回调tcp_client_connected()函数
    ...
    while(res==0)
    {
        ...
        if(tcp_client_flag&1<<6) {           //是否收到数据?
            ...
            printf("%s",tcp_client_recvbuf); //显示接收到的数据
